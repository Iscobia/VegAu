		...
		if sheet_name == 'PRAvirgin': # PRAvirgin is the only sheet to have only 1 row with headers
			...
			if row == 0 :
				...
				if sheet_name == 'PLANTS':
					key = doc.sheets['PLANTS'][1,col].value
					imported_data[key] = {}
					imported_data[key]['CROP_index']				= row			# CROProw
					imported_data[key]['colIndex_PRAyields']		= row +	2		# CROPcol_yields
					imported_data[key]['colIndex_PRAedibility']		= row + 6		# CROPcol
				...
				if sheet_name == 'NUTRITION' or sheet_name == 'NUTRITION_virgin':
					...
					imported_data[key] = {}
					imported_data[key]['CROP_index']	= row	
