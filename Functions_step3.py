...
def ASSESS_FoodNutrients(x, nutrition):
	...
	*	nutrition	is the dictionary that has been imported from 'input[COUNTRY].py'
	...
	for crop in x.totalYields.keys(): # for each column/crop in PRAyields
		total_yield			=	float(x.totalYields[crop]) * 1000		# converting the total Yields from tons to kilogramms
		x.rotation_in_years	=	(x.rotation_length[PRA]/12)
		yearly_Yield =  total_yield / x.rotation_in_years 	# in kg (total_yield is in kg)
		x.DailyResources[crop] = ( ( x.totalYields[crop] * prodQUANTITY(crop) )/ 365 ) / totalPopulation
